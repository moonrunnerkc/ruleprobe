 ...
 *
 * Resolves the Anthropic API key from CLI flags, environment variables,
 * and the .ruleprobe/config.json file (in that priority order).
 */
 ...
 *
 * Priority order for Anthropic API key:
 *   CLI flag --anthropic-key > env var ANTHROPIC_API_KEY > .ruleprobe/config.json
 *
 * @param projectDir - Root directory of the project being analyzed
 * @param cliOptions - Options from CLI flags
 * @returns Resolved semantic analysis config, or null if no API key is available
 */
